...
const searchScreen = ({navigation}) => {
    ...
    const renderSearchedCarparks = (cpArray, searchTerm) => {
        ...
        for (var item in cpArray) {
            ...
            if (cpArray[item].address.toLowerCase().includes(searchTerm)) {
    ...
    return (
        <View style={{height:'100%',backgroundColor:'#fff'}}>
            ...
            <View style={{backgroundColor:'#f0f0f0',width:'92%',height:50,margin:15,marginBottom:5,borderRadius:5,alignItems:'center',flexDirection:'row'}}>
                ...
                <TextInput onChangeText={searchTerm => searchTerm.length >= 3 ? renderSearchedCarparks(CarparksArray,searchTerm.toLowerCase()) : ''} style={[styles.medium,{fontSize:18,width:'90%',height:50,marginLeft:10,color:'#404040'}]} placeholder="Search by Address or Carpark No." placeholderTextColor="#777" />
            </View>
            <View>
                {searchedCarparks}
            </View>
